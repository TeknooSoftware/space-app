...
- Update API 
  ...
  - Add API `/api/v1/login` JSON login with specific keys generated by the user in the WebUI. This api endpoint is not
    protected by JWT token. The json body must contain an object with attribute `username` and `token`. The token is the
    value generated in the new API Keys view, the username must be following `[token name]:[email]`.
...
- Update libraries:
  ...
  - Symfony 7.2.5
  - Endroid/qr-code
  - Doctrine & Mongodb Odm Bundle and Mongo extension v2
  - Require Doctrine Persistence 4 
  - Omines Oauth2
  - Teknoo States, Recipe and East Foundation
  - Teknoo East PaaS 4.4
...
- Fix doctrine odm mapping types
- Fix some issue in compose.yml and Makefile
- Fix typo in AbstractVarsMapper.php
