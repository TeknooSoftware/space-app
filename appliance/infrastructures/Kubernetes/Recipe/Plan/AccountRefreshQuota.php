...
/**
 * Kubernetes quota refresh of a single environment: re-apply the account's quotas on the `kubeNamespace` of
 * the cluster `clusterName` (both put in the workplan by the caller from the current `AccountEnvironment`).
 * Executed in the `new_task` worker through the `Teknoo\Space\Recipe\Plan\Task\AccountRefreshQuotaTask` plan,
 * which loads the account, its history, clusters and environments, then loops over every environment and
 * delegates here for each one hosted on a Kubernetes cluster.
 *
 * @copyright   Copyright (c) EIRL Richard Déloge (https://deloge.io - richard@deloge.io)
 * @copyright   Copyright (c) SASU Teknoo Software (https://teknoo.software - contact@teknoo.software)
 * @license     http://teknoo.software/license/bsd-3         3-Clause BSD License
...
class AccountRefreshQuota implements EditablePlanInterface
{
    ...
    protected function populateRecipe(RecipeInterface $recipe): RecipeInterface
    {
        ...
        $recipe = $recipe->require(new Ingredient('string', 'kubeNamespace'));
