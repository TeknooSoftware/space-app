...
Feature: Web interface to allow users to login
  ...
  Scenario: From the UI, test login via the recovery method with an existant email, the platform must send a notification and login when subscribed user follow the sent link
    Given an account for "My Company" with the account namespace "my-company"
    And a user, called "Dupont" "Jean" with the "dupont@teknoo.space" with the password "Test2@Test"
    And the platform is booted
    When a user goes to the recovery request page
  ...
  Scenario: From the UI, test login via the recovery method with an existant email of a user with 2FA enables, the platform must send a notification and login when subscribed user follow the sent link and input a valid TOTP token.
    Given an account for "My Company" with the account namespace "my-company"
    And a user, called "Dupont" "Jean" with the "dupont@teknoo.space" with the password "Test2@Test"
    And the 2FA authentication is enabled for the last user
    And the platform is booted
