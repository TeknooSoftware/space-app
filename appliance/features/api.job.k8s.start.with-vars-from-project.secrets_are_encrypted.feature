...
Feature: API endpoints to create new job and deploy project with variables from project's configuration
  ...
  Scenario: From the API, execute a job from an owned project, with project's encrypted variables, prefix, a valid paas file, using conditions via a request with a json body
    Given encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
    And a git cloning agent
  ...
  Scenario: From the API, execute a job from an owned project, with project's encrypted variables, prefix, a valid paas file, using conditions, encrypted messages between workers, via a request with a json body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
  ...
  Scenario: From the API, execute a job from an owned project, with project's encrypted variables, prefix, a valid paas file, jobs via a request with a json body
    Given encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
    And a git cloning agent
  ...
  Scenario: From the API, execute a job from an owned project, with project's encrypted variables, prefix, a valid paas file, jobs, encrypted messages between workers, via a request with a json body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
