Feature: On a space instance, we can start a job from a project, User can define some variable in the project
  ...
  Scenario: Execute a job from a project, with project's vars and it takes too long
  ...
  Scenario: Execute a job from a project, with encrypted project's vars with prefix and paas file is valid
    Given A Space app instance
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
