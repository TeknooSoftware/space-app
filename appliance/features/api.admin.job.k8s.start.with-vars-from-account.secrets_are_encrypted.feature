...
Feature: API admin endpoints to create new job and deploy project with encrypted secrets from account's configuration
  ...
  I want to create new jobs from account's projects to deploy them with encrypted secrets from the account's
  configuration
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, prefix, a valid paas file, via a request with a form url encoded body
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variable, prefix, a valid paas file, via a request with a form url encoded body
    Given encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
    And a git cloning agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's two encrypted variables, prefix, a valid paas file, encrypted messages between workers, via a request with a form url encoded body
  ...
  Scenario: From the API, as Admin, re-execute a job from a project, with account's encrypted variables, prefix, a valid paas file, encrypted messages between workers, via a request with a form url encoded body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, prefix, a valid paas file, via a request with a json body
    Given encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
    And a git cloning agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, prefix, a valid paas file, encrypted messages between workers, via a request with a json body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, prefix, defined quota, without defined resources, a valid paas file, via a request with a json body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, prefix, defined quota, with partial defined resources, a valid paas file, via a request with a json body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, prefix, defined quota, with full defined resources, a valid paas file, via a request with a json body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, prefix, defined quota, with required resources exceeded quota, a valid paas file, via a request with a json body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, prefix, a valid paas file with default generic values for variables and all variables are not filled via a request with a json body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, a valid paas file, with new variables to persist after run into the project, via a request with a form url encoded body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, a valid paas file, with new variables to persist after run into the project, via a request with a json body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, prefix, a valid paas file, using conditions via a request with a json body
    Given encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
    And a git cloning agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, prefix, a valid paas file, using conditions encrypted messages between workers, via a request with a json body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, prefix, a valid paas file, jobs via a request with a json body
    Given encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
    And a git cloning agent
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variables, prefix, a valid paas file, jobs encrypted messages between workers, via a request with a json body
    Given encryption capacities between servers and agents
    And encryption of persisted variables in the database
    And a kubernetes client
    And a job workspace agent
