Feature: API admin endpoints to create new job and deploy project with encrypted secrets from account's configuration
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's encrypted variable, prefix, a valid
  paas file, via a request with a form url encoded body
    Given A Space app instance
    And encryption of persisted variables in the database
    And a kubernetes client
  ...
  Scenario: From the API, as Admin, execute a job from a project, with account's two encrypted variables, prefix,
  a valid paas file, encrypted messages between workers, via a request with a form url encoded body
    Given A Space app instance
