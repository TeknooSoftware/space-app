...
class LoadPersistedVariablesForJob
{
    public function __construct(
        private readonly AccountPersistedVariableLoader $loaderAccountPV,
        private readonly ProjectPersistedVariableLoader $loaderPV,
    ) {
    }
    ...
    private function loadFromAccount(
        ...
        $fetchedFromAccountPromise = new Promise(
            /** @var AccountPersistedVariable[] $apvs */
            static function (iterable $apvs) use ($newJob): void {
                /** @var AccountPersistedVariable[] $apvs */
                foreach ($apvs as $var) {
                    $newJob->envName ??= $var->getEnvName();
    ...
    private function loadFromProject(
        ...
        $fetchedFromProjectPromise = new Promise(
            /** @var ProjectPersistedVariable[] $persistedVariables */
