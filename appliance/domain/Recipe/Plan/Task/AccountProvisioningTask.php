...
/**
 * Worker-side plan of a single-environment account provisioning task (registry or environment install /
 * reinstall), reached through the `NewTaskRecipeRegistry` when one of the `Teknoo\Space\Object\DTO\Task\*`
 * tasks is consumed off the `new_task` transport. One instance is configured per task class, with the
 * type-dispatch `ProvisioningPlanBowl` of the matching role. The quota refresh, which spans every environment
 * of the account, has its own plan: {@see AccountRefreshQuotaTask}.
 *
